	...
	switch (ide) {
		...
		case 'codex_cli': {
			return [
				...
				'   ```',
				'   Field penting:',
				'   - `model_provider` (top-level) = key dari `[model_providers.<id>]` block (contoh: `9router` / `groupy`).',
				'   - `model` (top-level) = model id persis sesuai API proxy (`auto` ok).',
				'   - `env_key` = **WAJIB** — nama env var yang berisi API key (Bearer). Tanpa ini Codex sering “nolak” / reconnect.',
				...
				'   wire_api = "responses"',
				'   ```',
				`   Lalu set: export OPENAI_API_KEY="${apiKey}" (atau di Windows User env) lalu restart Codex.`,
