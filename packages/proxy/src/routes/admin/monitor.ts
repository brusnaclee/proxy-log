...
monitor.post("/monitor/sweep", async (c) => {
  ...
  // Scheduled/auto callers must not burn credits when Off. Dashboard "Test All" sends manual:true.
  if (mode === "off" && !manual) {
    return c.json(
      {
        error:
          "Model Monitor Auto Mode is Off — scheduled probes are disabled. Use Test All (manual) from the dashboard, or set Notif only / On in Settings.",
        monitorAutoMode: mode,
      },
      403,
    );
  ...
  console.log(
    `[monitor-sweep] started manual=${manual} mode=${mode} (mode=off keeps published catalog; only probe fields update)`,
  );
  ...
  return c.json({
    ...
    message: manual
      ? mode === "off"
        ? "Manual sweep started (mode Off — probe status only, published catalog unchanged)"
        : "Manual sweep started"
      : "Sweep started in background",
    monitorAutoMode: mode,
    manual,
  });
});
