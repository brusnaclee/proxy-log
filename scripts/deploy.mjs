...
async function main() {
  ...
  const quotaUpdate = await ssh.execCommand(
    "sudo -u postgres psql -d monit_api -c \"ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS rate_window_start text; ALTER TABLE admin_config ADD COLUMN IF NOT EXISTS token_limit_weight_percent integer NOT NULL DEFAULT 100; ALTER TABLE admin_config ADD COLUMN IF NOT EXISTS token_limit_weight_mode text NOT NULL DEFAULT 'first_rest_flat'; ALTER TABLE admin_config ADD COLUMN IF NOT EXISTS token_limit_weight_custom text NOT NULL DEFAULT '[]'; ALTER TABLE admin_config ADD COLUMN IF NOT EXISTS addon_required_models text NOT NULL DEFAULT '[]'; ALTER TABLE admin_config ADD COLUMN IF NOT EXISTS pro_role_id text DEFAULT '1354682701453725857'; ALTER TABLE addons ADD COLUMN IF NOT EXISTS default_duration_days integer NOT NULL DEFAULT 0; UPDATE admin_config SET trial_model_selection_mode = 'all' WHERE trial_model_selection_mode = 'all_gpy'; UPDATE admin_config SET token_limit_weight_mode = 'first_rest_flat' WHERE token_limit_weight_mode IS NULL OR token_limit_weight_mode = ''; UPDATE admin_config SET token_limit_weight_percent = 100 WHERE COALESCE(token_limit_weight_percent, 10) < 100; UPDATE admin_config SET global_daily_input_token_limit = 20000000 WHERE global_daily_input_token_limit = 2000000; UPDATE admin_config SET global_daily_output_token_limit = 50000000 WHERE global_daily_output_token_limit = 5000000; UPDATE admin_config SET global_per_model_prompt_limit = 3 WHERE global_per_model_prompt_limit IN (5, 10); UPDATE model_limits SET prompt_limit = 3 WHERE prompt_limit IN (5, 10); UPDATE addons SET default_duration_days = 15 WHERE name = 'vibecode-5m' AND COALESCE(default_duration_days,0) = 0; UPDATE addons SET default_duration_days = 30 WHERE name = 'vibecode-10m' AND COALESCE(default_duration_days,0) = 0; UPDATE addons SET is_active = false WHERE name = 'vibecode-3m';\" 2>&1"
  );
